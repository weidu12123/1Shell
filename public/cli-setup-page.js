...
(() => {
  ...
  function bindEvents() {
    ...
    cliList.addEventListener('click', (e) => {
      ...
      else if (action === 'config-engine') openSkillApiModal();
    });

    // 初始检查 1Shell AI 引擎 API 状态
    loadSkillApiStatus();
  }
  ...
  async function loadSkillApiStatus() {
    try {
      ...
      if (active && active.apiKeySet) {
        state.skillApiReady = true;
        state.skillApiInfo = `${active.name || 'default'} · ${active.model || ''}`;
      } else if (providers.length > 0) {
        state.skillApiReady = false;
        state.skillApiInfo = 'API Key 未设置';
      } else {
        state.skillApiReady = false;
        state.skillApiInfo = '';
      }
  ...
  async function openSkillApiModal() {
    ...
    proxyModalTitle.textContent = '配置 1Shell AI 引擎';
    proxyModalSubtitle.textContent = '驱动主控台 AI / IDE 创作 / Skill & Playbook 执行（支持 OpenAI 兼容 / Anthropic）';
  ...
  function renderTools() {
    ...
    // 1Shell AI 引擎卡片（始终显示在最前面）
    const showEngine = state.filter === 'all' || state.filter === 'sandboxed';
    const engineCard = showEngine ? shellEngineCardHTML() : '';

    if (filtered.length === 0 && !showEngine) {
      cliList.innerHTML = '<div class="text-center text-slate-400 text-xs py-12">没有匹配的 CLI 工具</div>';
      return;
    }

    cliList.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">${engineCard}${filtered.map(toolCardHTML).join('')}</div>
  ...
  function shellEngineCardHTML() {
    const st = state.skillApiReady
      ? { cls: 'status-connected', icon: '●', text: '已配置' }
      : { cls: 'status-detected', icon: '○', text: '待配置' };
    const proxyInfo = state.skillApiInfo
      ? `<span class="text-emerald-600">🔌 ${escapeHtml(state.skillApiInfo)}</span>`
      : '<span class="text-amber-500">🔌 API 未配置 — 主控台 AI / IDE 创作 / Skill 运行均需此配置</span>';

    return `
      <div class="cli-card ${state.skillApiReady ? 'connected' : ''} p-4 rounded-xl bg-white dark:bg-[#0b1324] md:col-span-2">
        ...
        <div class="mt-3 text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2">1Shell 原生 AI 引擎，驱动主控台对话、IDE 工作台创作、Skill/Playbook 执行。支持 Anthropic 和 OpenAI 兼容协议。</div>
        <div class="mt-1.5 text-[10px]">${proxyInfo}</div>
        <div class="mt-3 flex items-center gap-1.5">
          <button data-action="config-engine" class="flex-1 h-7 rounded-lg border border-cyan-200 bg-cyan-50 text-cyan-600 text-[11px] font-semibold hover:bg-cyan-100 dark:bg-cyan-500/10 dark:border-cyan-500/30 dark:text-cyan-400">⚙ 配置 API</button>
        </div>
      </div>`;
  }
