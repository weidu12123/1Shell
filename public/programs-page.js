(() => {
  ...
  function bindSocket() {
    ...
    socket.on('guardian:render', (msg) => {
      ...
      // 同时推到结果 Tab，让诊断报告在「📊 结果」里可见
      if (msg.programId && msg.hostId) {
        pushResult({ programId: msg.programId, hostId: msg.hostId, stepId: 'guardian', payload: msg.payload });
      }
    });
  ...
  function renderStatusBadge(status) {
    ...
    if (status === 'warning') return '<span class="badge badge-warn">⚠ 已报告</span>';
