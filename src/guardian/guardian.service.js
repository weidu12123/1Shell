...
function buildGuardianSystem({ program, host, allowedSkills }) {
  ...
  const lines = [
    ...
    `2. 必要时用 execute_command 诊断（最多 ${MAX_GUARDIAN_TURNS - 2} 轮）`,
    `3. 能自己修复就用 execute_command 修`,
    `4. 涉及不确定或破坏性操作 → ask_user 请示`,
    `5. 诊断报告/修复摘要通过 render_result 推给用户`,
    `6. **必须**最后调用 report_outcome 宣告 resolved 或 unresolvable`,
    ``,
    `## 硬约束（绝对禁止）`,
    `- 不得操作名称含 "1shell" 的资源`,
    ...
    `## 响应风格`,
    `- 极简，不解释过程`,
    `- 优先用白名单 Skill 里已有的诊断流程`,
    `- 无法修复不要硬修，直接 report_outcome=unresolvable 告诉用户该人工介入`,
  ];
...
function createGuardianService({
  ...
  async function escalate({ program, hostId, failingStep, failureReason, execResult, runId, triggerId }) {
    ...
    // ─ 加载白名单 Skill ─
    const allowedSkills = loadAllowedSkills(skillRegistry, program.guardian?.skills || []);
      ...
      for (let turn = 0; turn < MAX_GUARDIAN_TURNS; turn++) {
        ...
        for (const tu of toolUses) {
          ...
          if (tu.name === 'execute_command') {
            ...
            io?.emit?.('guardian:render', { sessionId, toolUseId: tu.id, payload: tu.input });
            toolResults.push({
              type: 'tool_result',
              tool_use_id: tu.id,
              content: 'OK - rendered',
