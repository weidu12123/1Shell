...
function createBridgeService({ hostService, auditService, sshPool, sshShellPool }) {
  ...
  function execLocal(command, timeout, { source, hostId, hostName, clientIp }) {
    return new Promise((resolve) => {
      ...
      try {
        if (isWindows) {
          // 优先尝试 WSL bash（支持 Linux 脚本），没有 WSL 才回落到 PowerShell
          try {
            child = spawn('wsl.exe', ['bash', '-s'], { windowsHide: true, env: process.env });
          } catch {
      ...
      child.stdout.on('data', (chunk) => { if (stdout.length < MAX_BUFFER) stdout += chunk.toString(); });
      child.stderr.on('data', (chunk) => { if (stderr.length < MAX_BUFFER) stderr += chunk.toString(); });
