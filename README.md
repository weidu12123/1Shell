...
- **一端御万机**：一端登录，集中接管你的整个计算舰队。
- **万物皆可连（极致穿透）**：支持强大的中继（ProxyJump）级联！只需一台拥有公网 IP 的跳板机，无论是家里的 NAS、公司的内网电脑，还是无公网 IP 的云电脑，全部都能被 1Shell 丝滑穿透接管。
- **一条 SSH 链路**：复用终端、探针、文件浏览与 AI 协作。
- **一个控制面**：统一承载站点入口、运维视角与智能运维。
- **绝对零侵入**：目标机器不安装任何 Agent，保持 100% 纯净。

传统 Web SSH 解决的是"能连上公网鸡"。1Shell 要解决的是**"如何穿透网络结界，高效、持续、安全地管理你的所有数字资产"**。
...
| **中继穿透连接** | 以公网机器为跳板，无缝连接内网 PC、云电脑、NAS 等无公网设备 |
...
| **自选范围纠错** | 终端内选定指定范围代码或日志，让 AI 辅助精准分析与纠错 |
